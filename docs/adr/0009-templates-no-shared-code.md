...
4. **Each template directory is self-contained, imports only `@systna/sandcastle`** (chosen). One-line rule, easy to apply on review, keeps `init` a pure directory copy, and lets each template evolve independently.
