...
- **Dirty worktree** → throw with a message that names the path and suggests `commit` / `stash` / `discard` or `git worktree remove`.
...
- Users with in-progress work in a worktree get a clear, actionable error instead of silent clobbering.
